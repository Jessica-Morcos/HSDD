...
export default function EnterSymptomsPage() {
  ...
  const patientId = localStorage.getItem("patientId");   // ✅ FIXED

  useEffect(() => {
    console.log("🔥 EnterSymptomsPage loaded with:");
    console.log("token:", token);
    console.log("patientId:", patientId, "type:", typeof patientId);   // ✅ FIXED
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await submitSymptoms(token, patientId, text, tags);  // ✅ FIXED
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="items-center justify-between mb-8 bg-[#b0372b] p-4 relative">
        <PatientNav activePage="Enter Symptoms" />
      </header>

      <div className="m-6">
        <h1 className="text-3xl font-bold mb-4">Enter Symptoms</h1>
