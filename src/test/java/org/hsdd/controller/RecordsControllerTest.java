...
class RecordsControllerTest {
    ...
    // -------------------------------------------------------------------------
    // PREDICTIONS (PredictionWithAnnotationDto)
    // -------------------------------------------------------------------------

    @Test
    void listPredictions_returnsList() throws Exception {

        PredictionWithAnnotationDto p = new PredictionWithAnnotationDto(
                5L,
                10L,
                "HSDD",
                0.83,
        ...
        mockMvc.perform(get("/api/records/predictions")
                ...
                // FLAT JSON FIELDS (your backend output)
                ...
                .andExpect(jsonPath("$[0].confidenceLevel").value("LOW"))

        // annotations DO NOT EXIST — PredictionWithAnnotationDto has NO list in your code
        // so we remove assertions for "annotations"
        ;
    }
